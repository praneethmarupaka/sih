...
import type { ExtractedField, ComplianceReport } from '../types/compliance';
...
export function extractFields(
  ...
  // 2. MRP (Supports commas e.g. 1,499.00)
  const extractMRP = (): { value: string | null; confidence: number; reason?: string } => {
    ...
    if (match && match[1]) {
      ...
      const hasTaxes = /incl(?:usive)?\.?\s*of\s*all\s*taxes/i.test(cleanText);
      const displayVal = `₹ ${price}${hasTaxes ? ' (incl. of all taxes)' : ''}`;
      return {
        value: displayVal,
        confidence: findConfidenceNear(match[0]),
      };
  ...
  const extractNetQuantity = (): { value: string | null; confidence: number; reason?: string } => {
    // Pattern: Net Qty/Net Weight/Net Vol or simply numbers with metric units
    ...
    // Metric units preceded by number
  ...
  const extractMfgDate = (): { value: string | null; confidence: number; reason?: string } => {
    // Pattern near Mfg date, Pkd, packed on, date of mfg
    ...
    // Best before or expiry fallback
    ...
    // Standalone Month/Year like 08/2026 or AUG 2025
  ...
  const extractConsumerCare = (): { value: string | null; confidence: number; reason?: string } => {
    ...
    // Phone / Helpline (1800 toll free or 10 digit)
    ...
    // Generic consumer care phrase if mentioned without direct number
  ...
  const extractCountryOfOrigin = (): { value: string | null; confidence: number; reason?: string } => {
    ...
    // Direct check for common countries
  ...
  const extractManufacturerAddress = (): { value: string | null; confidence: number; reason?: string } => {
    if (category === 'Electronics') {
      // Check if QR code is mentioned or detected
  ...
  for (const rule of rules) {
    ...
    if (!rule.required) {
      ...
      // If OCR confidence is < 65%, flag for Manual Review (Yellow)
    ...
    fields.push({
      ...
      reason: status === 'violation' ? (extraction.reason || 'Missing required declaration') : (status === 'manual_review' ? 'OCR confidence below 65% - requires visual verification' : undefined),
      suggestion: status === 'violation' ? rule.suggestionIfMissing : undefined,
      notes: rule.notes,
